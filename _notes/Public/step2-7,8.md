...
> Note. 다음 코드의 버전은 Lazy Evaluation이 기본으로 적용된 오래된 `1.x` 버전이고, Eager Execution이 적용된 `2.x` 버전에서는 작동하지 않는다. 따라서 본 코드를 실행하고 싶다면 `tensorflow` 버전을 `1.x`로 바꾸거나, 다음 방법을 따르면 좋다.
> 
> 본 코드의 
> ```python
